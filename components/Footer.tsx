  ...
  return (
    <footer className="flexCenter mb-24">
      <div className="padding-container max-container flex w-full flex-col gap-14">
              ...
              <FooterColumn title={SOCIALS.title}>
                <ul className="regular-14 flex gap-4">
                  {SOCIALS.links.map((link) => (
                    <Link href='/' key={link.iconName}>
                    <FontAwesomeIcon icon={link} width={24} height={24}/>
                    </Link>
                  ))}
                </ul>
              </FooterColumn>
        ...
        <p className="regular-14 w-full text-center text-gray-30">2024 WonderLust | All rights reserved</p>
      </div>
    </footer>
  );
};
